...
Check if `insight-mine-YYYY-MM-DD.md` exists in the project (from a recent insight-miner run). If it does, read it and tell the user:

> I found insight-mine from [date]. I can use those verified angles as the basis for your content matrix. Want me to use it, or start fresh?

If the user says yes, skip to Step 2 — the insights become the seed for the matrix.

If no insight file exists, or the user wants to start fresh, check the project for about-me.md. If it exists, read it and pre-fill the description of who the user is. Skip that question and tell the user what you pulled.
...
If the user has more than 5 pillars, help them collapse. Propose groupings that merge related pillars into broader ones. Aim for 3–5. Show the mapping so the user can see how their ideas were combined and adjust if needed.
...
- **Claude Code / file-system surface (has Write tool):** ask the user where to save:
  > I can save this matrix as `content-matrix-YYYY-MM-DD.md`. Where should I put it? (Give me a directory path or say "here" for the current project root.)
  Once confirmed, save the file and confirm the path.
...
After saving, record the save location in memory so future content-matrix runs default to the same directory.
