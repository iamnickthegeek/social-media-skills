...
Before building the carousel, read `references/bleeding-edge-formats.md` from the social-media-skills directory. Use the LinkedIn carousel template to inform:
- Slide structure (hook → context → framework → summary → reply bait CTA)
- Optimal slide count (8–12; recommend based on content density)
- Tone guidance (conversational, not academic; each slide ends mid-thought)
- CTA approach (reply bait that drives conversation signals)
...
- Always read the format reference (Step 0) before building the brief.
