...
Before scoring, read `references/bleeding-edge-formats.md` from the social-media-skills directory. Use the LinkedIn benchmarks to:
- Compare the draft's hook type against top-performer distribution (number-led 31%, bold claim 27%, contrarian 18%)
- Check post length against the 180–250 word sweet spot for top 10%
- Evaluate CTA patterns against known high-performers (reply bait, newsletter mention)
- Use the cross-platform engagement table for benchmark comparisons
...
[
  {
    ...
    "options": [
      ...
      {"label": "Use benchmark data", "description": "Score against June 2026 platform benchmarks from the format reference. Good fallback."},
...
If "Use benchmark data":
Use the benchmarks from the format reference file loaded in Step 0.
...
- Has the post been run through the `humanizer` skill? If not, score this criterion 3 or below automatically — AI-isms are present.
...
- Are there any red flags: banned words listed in voice.md's absence patterns, generic phrases, corporate tone, signposting ("Here's what changed", "Let's explore"), promotional language?
- Is it the right length compared to their top performers?

**Humanizer requirement:** Before declaring a post ready to publish, run it through the `humanizer` skill (see creative/humanizer). This strips AI-isms the first-pass writing missed. If the scorer is applied to a post that hasn't been humanized, flag it in the scorecard and offer to run the humanizer as the first fix.
...
After the scorecard, save the full scoring output to a markdown file:

1. Check if a project directory already exists (e.g. `~/.hermes/projects/<project-name>/content-research/`). If found, offer it as the default.
2. If no project directory exists, ask the user where to save.
3. The filename should be `post-score-YYYY-MM-DD.md`.
4. After saving, confirm the full file path to the user.
5. Record the save location in memory so future post-scorer runs default to the same directory.
...
When the user picks "Use benchmark data" and the format reference has been loaded, use those benchmarks directly. If the reference file is unavailable, use these June 2026 platform benchmarks as fallback:
...
> **Note:** These fallback benchmarks are sourced from the same format reference. When you update `references/bleeding-edge-formats.md`, these defaults become stale — prefer loading the reference file (Step 0) over hardcoding.
