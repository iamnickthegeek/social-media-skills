...
### Handling long-form samples (books, manuscripts)

If the user provides book chapters or full manuscripts (25,000+ words), do not read every word. Request:
- 1 chapter (~3,000–4,000 words) for natural prose rhythm
- 1 intro from a different book (~1,500–2,000 words) for how they frame and open ideas
- 1 additional sample of any kind for range

This keeps analysis focused and avoids drowning in content.

### When the user dislikes their own voice

If the user says they don't like their writing style or want more "oomph," do not faithfully reproduce the samples. Instead:

1. Extract the foundational voice signals from their samples (sentence rhythm, humour style, asides, tone)
2. Identify 2–3 reference writers whose structural patterns (not tone) fill the gaps the user wants
3. Build voice.md as a hybrid: the user's natural foundation + structural patterns from references
4. Be transparent about which reference patterns you're borrowing and why
5. Let the user approve or push back on reference choices before writing voice.md

This is a common pattern: instructional non-fiction authors often have a solid "teacher voice" but lack the editorial edge (strong hooks, contrarian openings, sharp closers) needed for social media. The skill should bridge that gap, not just document what exists.
...
- **AskUserQuestion tool not available on all surfaces.** On Telegram and similar text-only platforms, the AskUserQuestion form tool does not render. Fall back to asking questions as numbered lists with options in chat text. The user types their answer; accept free-form responses that don't match the options.
- **Never fabricate voice signals.** If a pattern is not in the samples, say so. Do not invent "signature phrases" or "absence signals" from generic banned-words templates. Every item in voice.md must be backed by observation.
- **British English throughout unless the samples are clearly American.**
